...
**A scope note on `run_repl`, worth understanding before you consider this task "fully tested from both entry points":** an earlier version of this note claimed a REPL-stdin-typed reproduction of the newline-forging bug was structurally impossible, reasoning that `run_repl` reads lines via `std::io::BufRead::lines()`, which splits strictly on `\n` before `main.rs` ever sees a line. That reasoning is correct as far as it goes — no *raw* newline byte typed at the REPL can survive `BufRead::lines()` — but it is incomplete, and a whole-branch review of this plan caught the gap: `fslite-command`'s lexer has its own double-quote reader (`read_double_quoted` in `crates/fslite-command/src/lexer.rs`) that translates the two-*character* escape sequence `\n` (backslash, then the letter `n` — not a raw newline byte) into an actual newline *character* in the parsed string. This means a REPL user *can* construct a `Command` whose path field contains a real newline, entirely through a normal, newline-free line of stdin input, by double-quoting a path containing that escape sequence, e.g. `mkdir "/evil\nerror: quota exceeded (InternalStorageFailure).txt"` (single-quoted segments are unaffected — `read_single_quoted` copies bytes verbatim with no escape translation). This route is now covered by `repl_mode_does_not_forge_an_extra_stderr_line_from_a_lexer_escaped_newline` in `crates/fslite-cli/tests/e2e_repl.rs`, which was verified RED against the old `sanitize_for_terminal` call site and GREEN against the `sanitize_name` fix. The remote-mode angle this note originally raised — a `RemoteExecutor` talking to a compromised or buggy `fslite-server`, whose JSON error response arrives over HTTP rather than through `BufRead::lines()` or the lexer at all — remains a genuinely separate, unverified channel; building a reliable test for it would still require confirming how a raw newline byte in a `VirtualPath` round-trips through URL construction (`reqwest`) and path extraction (`axum`) end to end. If remote-mode error-message injection via a hostile server turns out to matter to you, treat it as a follow-up investigation, not a checkbox this plan can honestly claim to close.
