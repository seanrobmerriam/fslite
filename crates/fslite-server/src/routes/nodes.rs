...
async fn stat_inner(
    ...
    let node = state
        ...
        .stat(
            &ctx,
            &path,
            StatOptions::default().follow_symlinks(follow_symlinks),
        )
        .await?;
...
fn revision_from(raw: Option<u64>) -> Result<Option<fslite_core::Revision>, ApiError> {
    match raw {
        None => Ok(None),
        Some(0) => Err(ApiError::MalformedBody(
...
async fn put_node(
    ...
    match params.get("type").map(String::as_str) {
        ...
        Some("symlink") => {
            let body: SymlinkBody = serde_json::from_slice(&body)
                .map_err(|e| ApiError::MalformedBody(e.to_string()))?;
            let target = LinkTarget::parse(&body.target)
        ...
        _ => Err(ApiError::MalformedBody(
            "query parameter `type` must be `directory` or `symlink`".into(),
        )),
    }
}
...
async fn post_action(
    ...
    match params.get("action").map(String::as_str) {
        Some("copy") => {
            let to = parsed.to.ok_or_else(|| {
                ApiError::MalformedBody("`to` is required for action=copy".into())
            })?;
        ...
        Some("move") => {
            let to = parsed.to.ok_or_else(|| {
                ApiError::MalformedBody("`to` is required for action=move".into())
            })?;
            let to = parse_path(&to)?;
        ...
        Some("trash") => {
            let options =
                fslite_core::MutationOptions::default().expected_revision(expected_revision);
            Ok(Json(state.fs.trash(&ctx, &from, options).await?).into_response())
        }
        _ => Err(ApiError::MalformedBody(
            "query parameter `action` must be `copy`, `move`, or `trash`".into(),
        )),
    }
}
...
async fn patch_node(
    ...
    match body {
        ...
        PatchBody::SetAttribute {
            ...
            let options = fslite_core::MutationOptions::default()
                .expected_revision(revision_from(expected_revision)?);
            Ok(Json(
                state
                    .fs
        ...
        PatchBody::RemoveAttribute {
            ...
            Ok(Json(
                state
                    .fs
                    .remove_attribute(&ctx, &path, &key, options)
                    .await?,
            ))
        }
    }
